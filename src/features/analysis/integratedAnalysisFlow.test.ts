...
function offer(id: string, title = 'Automation Specialist') { return { id, title, company: 'Example', sourceUrl: `https://example.test/${id}`, missingFields: [], warnings: [] } }
...
function repositoryFor(items: ReturnType<typeof report>[]) {
  ...
  return {
    importReport: vi.fn(async () => ({ importSessionId: `session-${repositoryForCalls++}` })),
...
describe('integrated analysis batch', () => {
  it('starts the visible batch once, automatically queues PASS/NEEDS_REVIEW and skips FAIL', async () => {
    ...
    const reports = [report('A', [offer('pass'), offer('fail', 'Niechciane stanowisko')])]
