...
describe('AnalysisOrchestrator', () => {
  it('fetches a normalized source before it calls AI and keeps a source fallback non-blocking', async () => {
    const progress: string[] = []
    const provider = { find: async () => ({ text: '', sourceQuality: 'unavailable' as const, source: { offerId: 'source-1', status: 'unavailable', sourceQuality: 'unavailable', requirements: [], responsibilities: [], benefits: [], missingInformation: [], warnings: [], fetchedAt: '2026-07-31T10:00:00.000Z', errorCode: 'SOURCE_TIMEOUT' as const }, sourceErrorCode: 'SOURCE_TIMEOUT' as const }) }
    const service = { analyze: async (_profile: unknown, current: FilteredJobOffer, content: { sourceQuality: string }) => { expect(content.sourceQuality).toBe('unavailable'); return analysis(current.offer.id) } }
    const repository = { load: async () => [], save: async () => undefined }
    const orchestrator = new AnalysisOrchestrator(provider as never, service as never, repository)
    await orchestrator.analyzeAll(defaultProfile, [item('source-1')], (entry) => progress.push(`${entry.state}:${entry.sourceErrorCode ?? ''}`))
    expect(progress).toEqual(['fetching:', 'analyzing:SOURCE_TIMEOUT', 'ready:SOURCE_TIMEOUT'])
  })
  ...
  it('reports a controlled save diagnostic instead of losing the analysis result silently', async () => { const service = { analyze: async (_profile: unknown, current: FilteredJobOffer) => analysis(current.offer.id) }; const repository = { load: async () => [], save: async () => { throw new Error('database') } }; const provider = { find: () => ({ text: '', sourceQuality: 'partial' as const }) }; const progress: string[] = []; const orchestrator = new AnalysisOrchestrator(provider as never, service as never, repository); const results = await orchestrator.analyzeAll(defaultProfile, [item('1')], (entry) => progress.push(`${entry.offerId}:${entry.state}:${entry.error ?? ''}`)); expect(results).toEqual([]); expect(progress).toContain('1:retry:ANALYSIS_SAVE_FAILED') })
})
