...
export function AppModeProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => ({ mode, session, loading, configured: supabaseConfigured, enterDemo: () => { sessionStorage.setItem(DEMO_KEY, 'demo'); setMode('demo') }, exitDemo: () => { sessionStorage.removeItem(DEMO_KEY); setMode(null) }, signOut: async () => { await supabase?.auth.signOut(); setSession(null); setMode(null) } }), [mode, session, loading])
