...
import { buildAnalysisIdentity } from '../analysis/analysisIdentity'
import { CURRENT_ANALYSIS_ALGORITHM_VERSION, CURRENT_ANALYSIS_MODEL_VERSION, CURRENT_ANALYSIS_PROMPT_VERSION } from './analysisQueue'
...
const empty = (): Store => ({ profile: null, activeImportSessionId: null, profileVersions: [], sessions: [], offers: [], versions: [], links: [], states: [], hardFilters: [], queue: [], workspaceAnalyses: [], analysisVersions: [], results: {}, viewed: [] })
...
const id = (prefix: string, seed: string) => {
  ...
  return `${prefix}-${(hash >>> 0).toString(36)}-${seed.replace(/[^a-z0-9]/gi, '').slice(-8) || Date.now().toString(36)}`
}
...
export function localWorkspaceRepository(userId = 'demo-user', storage: Pick<Storage, 'getItem' | 'setItem'> | null = typeof window === 'undefined' ? null : localStorage): WorkspaceRepository {
  ...
  return {
    ...
    async setActiveImportSession(importSessionId) { const store = read(); if (importSessionId !== null) { const session = store.sessions.find((item) => item.id === importSessionId); if (!session) throw new Error('WORKSPACE_IMPORT_NOT_FOUND'); if (session.status === 'reverted') throw new Error('WORKSPACE_ACTIVE_IMPORT_REVERTED') }; store.activeImportSessionId = importSessionId; write(store) },
    async listImportSessions() { return copy(read().sessions) },
    async revertImport(importSessionId) { const store = read(); const session = store.sessions.find((item) => item.id === importSessionId); if (!session) throw new Error('WORKSPACE_IMPORT_NOT_FOUND'); if (session.status !== 'reverted') { session.status = 'reverted'; session.revertedAt = new Date().toISOString(); session.operationMetadata = { ...session.operationMetadata, revertedAt: session.revertedAt }; if (store.activeImportSessionId === importSessionId) store.activeImportSessionId = null; write(store) }; return { importSessionId, status: 'reverted' } satisfies RevertImportResult },
    async reactivateImport(importSessionId) { const store = read(); const session = store.sessions.find((item) => item.id === importSessionId); if (!session) throw new Error('WORKSPACE_IMPORT_NOT_FOUND'); if (session.status === 'reverted') { session.status = session.invalidCount > 0 ? 'partial' : 'active'; session.reactivatedAt = new Date().toISOString(); session.operationMetadata = { ...session.operationMetadata, reactivatedAt: session.reactivatedAt }; store.activeImportSessionId = importSessionId; write(store) }; return { importSessionId, status: session.status === 'partial' ? 'partial' : 'active' } satisfies ReactivateImportResult },
    async enqueueAnalysis(offerId, options) {
      ...
      const analysisIdentity = await buildAnalysisIdentity({ userId, jobOfferId: offerId, offerVersionId: offer.currentVersionId, profileVersionId: store.profile.currentVersionId, promptVersion: CURRENT_ANALYSIS_PROMPT_VERSION, modelVersion: CURRENT_ANALYSIS_MODEL_VERSION, algorithmVersion: CURRENT_ANALYSIS_ALGORITHM_VERSION })
      ...
      const reusableVersion = !options?.forceReanalysis ? store.analysisVersions.find((item) => item.analysisIdentity === analysisIdentity && item.jobOfferId === offerId && item.offerVersionId === offer.currentVersionId && item.profileVersionId === store.profile!.currentVersionId) : undefined
      if (reusableVersion) {
        const completed = store.queue.find((item) => item.id === reusableVersion.queueItemId)
        if (completed) return { queueItem: copy(completed), idempotent: true, reused: true }
      }
      const now = new Date().toISOString(); const queueItem: AnalysisQueueItem = { id: id('analysis-queue', `${offerId}-${now}`), userId, jobOfferId: offerId, offerVersionId: offer.currentVersionId, profileVersionId: store.profile.currentVersionId, hardFilterResultId: hardFilter.id, status: 'queued', requestType: options?.forceReanalysis || store.workspaceAnalyses.some((item) => item.jobOfferId === offerId && item.latestVersionId) ? 'reanalysis' : 'initial', requestedBy: 'user', attemptCount: 0, maxAttempts: 2, lockedAt: null, leaseExpiresAt: null, workerToken: null, providerResponseId: null, analysisIdentity, lastError: null, queuedAt: now, startedAt: null, completedAt: null, cancelledAt: null, updatedAt: now }
      store.queue.push(queueItem); state.lifecycleStatus = 'selected_for_analysis'; touch(state); write(store)
      return { queueItem: copy(queueItem), idempotent: false, reused: false }
    },
    ...
    async completeLocalAnalysis(queueItemId: string, analysis: JobAnalysis) {
      ...
      const version: AnalysisVersion = {
        ...
        analysisData: analysis as unknown as Record<string, unknown>, hardFilterStatus: queueItem.hardFilterResultId ? (store.hardFilters.find((item) => item.id === queueItem.hardFilterResultId)?.status ?? 'pass') : 'pass', modelProvider: 'demo-fixture', modelVersion: CURRENT_ANALYSIS_MODEL_VERSION, promptVersion: CURRENT_ANALYSIS_PROMPT_VERSION, algorithmVersion: analysis.scoring?.algorithmVersion ?? CURRENT_ANALYSIS_ALGORITHM_VERSION, confidence: analysis.scoring?.criterionConfidence ?? null, coverage: analysis.scoring?.coverage ?? null, sourceType: 'demo-fixture', sourceQuality: 'fixture', analysisIdentity: queueItem.analysisIdentity, createdAt: now,
      }
