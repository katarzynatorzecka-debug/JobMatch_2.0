...
import { assertUniqueHardFilterItems, type ActiveAnalysisQueueItem, type HardFilterBatchInput, type LegacyAnalysisIssue, type ReactivateImportResult, type RevertImportResult, type WorkspaceImportInput, type WorkspaceRepository, type WorkspaceSnapshot } from './workspaceRepository'
...
export function supabaseWorkspaceRepository(user: User): WorkspaceRepository {
  ...
  const loadWorkspace = async (): Promise<WorkspaceSnapshot> => {
    const [profile, profileVersions, sessions, links, offers, versions, states, hardFilters, analyses, queue, workspaceAnalyses, analysisVersions, viewed] = await Promise.all([
      client.from('profiles').select('id,user_id,profile_data,current_version_id,active_import_session_id,created_at,updated_at').eq('user_id', user.id).maybeSingle(),
      client.from('profile_versions').select('id,user_id,profile_id,version_number,profile_data,content_hash,created_at').eq('user_id', user.id),
      client.from('import_sessions').select('id,user_id,source_type,source_filename,status,found_count,new_count,duplicate_count,invalid_count,needs_review_count,warnings,operation_metadata,created_at,reverted_at,reactivated_at').eq('user_id', user.id).order('created_at', { ascending: false }),
      client.from('import_offer_links').select('id,user_id,import_session_id,job_offer_id,offer_version_id,match_type,raw_external_id,dedup_evidence,is_new,is_duplicate,needs_review,created_at').eq('user_id', user.id), client.from('job_offers').select('id,user_id,source_type,source_url,normalized_source_url,canonical_fingerprint,title,company,location,current_data,source_data,first_seen_at,last_seen_at,current_version_id,created_at,updated_at').eq('user_id', user.id).not('current_version_id', 'is', null), client.from('offer_versions').select('id,user_id,job_offer_id,version_number,offer_data,content_hash,source_url,observed_at,import_session_id,created_at').eq('user_id', user.id), client.from('offer_user_state').select('id,user_id,job_offer_id,lifecycle_status,favorite,applied,exclusion_reason,state_metadata,excluded_at,restored_at,last_viewed_at,created_at,updated_at').eq('user_id', user.id),
      client.from('hard_filter_results').select('id,user_id,job_offer_id,offer_version_id,profile_version_id,status,reasons,missing_information,checked_criteria,algorithm_version,created_at,is_current').eq('user_id', user.id), client.from('job_analyses').select('job_offer_id,analysis_data').eq('user_id', user.id), client.from('analysis_queue').select('id,user_id,job_offer_id,offer_version_id,profile_version_id,hard_filter_result_id,status,request_type,requested_by,attempt_count,max_attempts,locked_at,lease_expires_at,worker_token,provider_response_id,analysis_identity,last_error,queued_at,started_at,completed_at,cancelled_at,updated_at').eq('user_id', user.id), client.from('workspace_job_analyses').select('id,user_id,job_offer_id,latest_version_id,created_at,updated_at').eq('user_id', user.id), client.from('analysis_versions').select('id,user_id,job_analysis_id,job_offer_id,offer_version_id,profile_version_id,queue_item_id,analysis_data,hard_filter_status,model_provider,model_version,prompt_version,algorithm_version,confidence,coverage,source_type,source_quality,analysis_identity,created_at').eq('user_id', user.id), client.from('recently_viewed').select('user_id,job_offer_id,viewed_at').eq('user_id', user.id),
    ])
  ...
  return {
    ...
    async listActiveAnalysisQueueItems(offerIds) {
      let query = client.from('analysis_queue').select('id,job_offer_id,status,last_error').eq('user_id', user.id).in('status', ['queued', 'processing'])
      if (offerIds?.length) query = query.in('job_offer_id', offerIds)
      const { data, error } = await query
      if (error) throw cloudError('Nie udało się odczytać statusu kolejki analizy.')
      return (data ?? []).map(mapActiveQueueItem)
    },
    ...
    async setActiveImportSession(importSessionId) { await rpc('workspace_set_active_import_session', { import_session_id: importSessionId }, 'Nie udalo sie zapisac aktywnej sesji importu.') },
    ...
    async listImportSessions() { const { data, error } = await client.from('import_sessions').select('id,user_id,source_type,source_filename,status,found_count,new_count,duplicate_count,invalid_count,needs_review_count,warnings,operation_metadata,created_at,reverted_at,reactivated_at').eq('user_id', user.id).order('created_at', { ascending: false }); if (error) throw cloudError('Nie udało się odczytać historii importów.'); return (data ?? []).map(mapSession) },
    ...
    async enqueueAnalysis(offerId, options) { return mapEnqueueResult(await rpc(options?.forceReanalysis ? 'workspace_reanalyze_analysis' : options?.allowHardFilterFail ? 'workspace_enqueue_analysis_override' : 'workspace_enqueue_analysis', { offer_id: offerId }, 'Nie udało się dodać oferty do kolejki AI.')) },
