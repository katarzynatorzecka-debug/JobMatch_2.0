...
export function semanticMappingToDraft(mapping: SemanticProfileMapping, source: CvSource): UserProfileDraft {
  ...
  intelligence.candidateFacts.projects = (facts.projects ?? []).filter((item) => item.status !== 'unknown').map((item) => ({ name: item.name, scope: item.scope, role: item.role, stack: item.stack, result: item.result, link: item.link, deployed: item.deployed, uxEvidence: item.uxEvidence, prototypingEvidence: item.prototypingEvidence, evidence: evidenceFor(item.evidence), ...qualityFor(item) }))
