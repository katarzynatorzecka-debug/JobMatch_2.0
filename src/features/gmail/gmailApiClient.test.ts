...
describe('gmailApiClient', () => {
  ...
  it('validates preview-only search results and sends filters to the Edge Function', async () => {
    ...
    await expect(client.search({ sender: 'no-reply@rocketjobs.pl' })).resolves.toEqual({ messages: [safePreview], nextPageToken: 'next' })
    expect(invoke).toHaveBeenCalledWith('gmail-search', { filters: { sender: 'no-reply@rocketjobs.pl' } })
    expect((await client.search({ sender: 'no-reply@rocketjobs.pl' })).messages[0]).not.toHaveProperty('id')
    expect((await client.search({ sender: 'no-reply@rocketjobs.pl' })).messages[0]).not.toHaveProperty('raw')
  })

  it('accepts imported reports without RAW or token fields and confirms their receipt', async () => {
    const imported = { receiptId: '123e4567-e89b-42d3-a456-426614174000', messageRef: 'opaque-ref', report: { ...importedReport, raw: 'private-raw' }, accessToken: 'private-token' }
    const safeImported = { receiptId: imported.receiptId, messageRef: imported.messageRef, report: importedReport }
    ...
    await expect(client.importSelected(['opaque-ref'])).resolves.toEqual({ reports: [safeImported] })
    await expect(client.confirmImport(imported.receiptId, '223e4567-e89b-42d3-a456-426614174000')).resolves.toBeUndefined()
    expect(safeImported).not.toHaveProperty('accessToken')
    expect(safeImported.report).not.toHaveProperty('raw')
    expect(invoke).toHaveBeenLastCalledWith('gmail-confirm-import', { receiptId: imported.receiptId, importSessionId: '223e4567-e89b-42d3-a456-426614174000' })
  })
  ...
  it('rejects a search response that exceeds the server-side result contract', async () => {
    ...
    await expect(createGmailApiClient(invoke).search({})).rejects.toMatchObject({ code: 'GMAIL_RESPONSE_INVALID' })
  })
