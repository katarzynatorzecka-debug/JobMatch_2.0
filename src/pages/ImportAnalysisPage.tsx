...
export function ImportAnalysisPage() {
  ...
  const [pipeline, setPipeline] = useState<PipelineState>(() => initialSession.current?.pipeline ?? 'idle'); const [pipelineError, setPipelineError] = useState(() => initialSession.current?.pipeline === 'partial_complete' ? t('import.error.refreshInterrupted') : '')
  ...
  useEffect(() => {
    ...
    void workspaceRepositoryFor('authenticated', session.user).loadWorkspace().then((snapshot) => {
      ...
      if (!restored) return
      setBatch(restored.batch); setPipeline(restored.pipeline); setProgress(restored.progress); setCounts(restored.counts); setPipelineError(restored.pipeline === 'partial_complete' ? t('import.error.partialRestore') : '')
    }).catch(() => { if (!cancelled) setPipelineError(t('import.error.restore')) }).finally(() => { if (!cancelled) setRestoredWorkspaceBatch(true); setRestoringWorkspace(false) })
  ...
  async function handleUrl() {
    ...
    if (!normalizedUrl) { setPipelineError(t('import.error.invalidUrl')); return }
    ...
    try {
      ...
      setPipelineError(code === 'UNSUPPORTED_SOURCE_DOMAIN' ? t('import.error.unsupportedUrl') : t('import.error.urlRead'))
  ...
  async function startAnalysis(reportsOverride?: BatchReport[]) {
    ...
    if (!reports.length) { setPipelineError(t('import.error.restoreOffer')); return }
    ...
    if (!profile) { analysisRunRef.current = false; setPipelineError(profileError ?? t('import.error.profileRequired')); return }
    ...
    } catch (cause) { setPipeline('idle'); setPipelineError(cause instanceof Error ? cause.message : t('import.error.start')) } finally { analysisRunRef.current = false }
  }
  async function retryOffer(item: IntegratedOfferProgress, force = false) {
    ...
    if (!profile) { setPipelineError(cloudProfile?.error ?? localProfile?.warning ?? t('import.error.profileSaveRequired')); return }
...
{isProcessingUrl && <SectionCard title={t('import.url.processingTitle')}><p className="field-hint">{t('import.url.processingCopy')}</p></SectionCard>}
    {isReviewing && <>
      <SectionCard title={t('import.review.title')} className="import-review-card">
        ...
        {pipelineError && <Alert title={t('import.review.analysisErrorTitle')} tone="warning">{pipelineError}</Alert>}
        ...
        <ul className="import-report-list" aria-label={t('import.reports.aria')}>
          {batch.entries.map((entry) => entry.kind === 'file_error' ? <li key={entry.id} className="import-report-list__error"><div><strong>{entry.fileName}</strong><span>{t('import.file.failed')}</span></div><Alert title={t('import.file.skipped')} tone="warning">{entry.message}</Alert></li> : <li key={entry.id}>
            ...
            <ul className="recognized-offers">{visibleOffers(entry).map((offer) => { const item = progress[progressKey(entry.id, offer.id)]; const issues = presentOfferIssues(offer); return <li className={`analysis-tile analysis-tile--${item?.state ?? 'waiting'}`} key={offer.id}><div><strong>{offer.title}</strong><span>{offer.company}{offer.sourceLabel ? ` · ${offer.sourceLabel}` : ''}{offer.location ? ` · ${offer.location}` : ''}</span>{issues.missing.length > 0 && <small className="offer-missing">{t('import.offer.missing')} {issues.missing.join(', ')}.</small>}{issues.warnings.length > 0 && <small className="offer-warning">{t('import.offer.review')} {issues.warnings.join(', ')}.</small>}<span className="analysis-tile__state">{statusLabel(item)}</span>{item?.state === 'processing' && <small>{t('import.offer.processing')}</small>}{item?.state === 'failed' && <><small className="analysis-tile__error">{item.error ?? t('import.offer.failed')}</small><SecondaryButton onClick={() => void retryOffer(item)}>{t('import.action.retry')}</SecondaryButton></>}{item?.state === 'completed' && <AnalysisPreview analysis={item.analysis} hardFilter={item.hardFilterStatus} freshness={item.freshness} analysisVersionId={item.analysisVersionId} />}{item?.state === 'rejected' && <><AnalysisPreview hardFilter="fail" /><SecondaryButton onClick={() => void retryOffer(item, true)}>{t('import.action.force')}</SecondaryButton></>}</div>{pipeline !== 'running' && !isFinished && <SecondaryButton onClick={() => setBatch((current) => removeBatchOffer(current, entry.id, offer.id))}>{t('import.action.removeOffer')}</SecondaryButton>}</li> })}</ul>
          </li>)}
        </ul>
...
function AnalysisPreview({ analysis, hardFilter, freshness, analysisVersionId }: { analysis?: JobAnalysis; hardFilter?: 'pass' | 'weak' | 'fail'; freshness?: IntegratedOfferProgress['freshness']; analysisVersionId?: string | null }) {
  ...
  return <div className="analysis-preview"><HardFilterStatusBadge status={hardFilter ?? analysis.hardFilterStatus} /><AnalysisQuality analysis={analysis} />{freshness && <small>{t('import.preview.freshness')} {freshness}</small>}{analysisVersionId && <small>analysis_version_id: {analysisVersionId}</small>}</div>
}
