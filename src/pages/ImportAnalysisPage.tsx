...
export function ImportAnalysisPage() {
  ...
  return <section className="page">
    ...
    {analysisProgress.length > 0 && <SectionCard title="Postęp analizy AI"><ul className="analysis-list">{analysisProgress.map((item) => <li key={item.offerId}><span>{item.offerId}</span><span className={`analysis-state analysis-state--${item.state}`}>{item.state === 'queued' ? 'oczekuje' : item.state === 'analyzing' ? 'analizowana' : item.state === 'ready' ? 'gotowa' : item.state === 'rejected' ? 'odrzucona przez Hard Filter' : 'wymaga ponowienia'}{item.error ? ` · ${item.error}` : ''}</span>{item.state === 'retry' && mode === 'authenticated' && <SecondaryButton disabled={analysisBusy} onClick={() => void startHardFilter(item.offerId)}>Ponów</SecondaryButton>}</li>)}</ul>{analysisCompleted && <div className="action-row"><PrimaryButton onClick={() => navigate('/offers')}>Zobacz wyniki</PrimaryButton></div>}</SectionCard>}
