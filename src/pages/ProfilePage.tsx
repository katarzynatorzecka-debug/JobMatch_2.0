...
export function ProfilePage() {
  ...
  return <section className="page page--profile-onboarding">
    ...
    {step === 'reading' && <ReadingStep usesSemanticMapper={mode === 'authenticated' && Boolean(session)} />}
...
function ProfileIntelligenceDetails({ profile }: { profile: UserProfile }) { const facts = profile.intelligence?.candidateFacts; if (!facts) return null; const rows: Array<[string, string]> = [['Doświadczenie', facts.experienceAreas.map((item) => item.area).join(', ')], ['Kompetencje i odpowiedzialności', facts.responsibilities.map((item) => item.capability).join(', ')], ['Domeny', facts.domains.map((item) => item.name).join(', ')], ['Osiągnięcia', facts.achievements.map((item) => item.capability).join(', ')], ['Języki', facts.languages.map((item) => item.level ? `${item.name} (${item.level})` : item.name).join(', ')], ['Edukacja', facts.education.map((item) => item.name).join(', ')], ['Certyfikaty', facts.certifications.map((item) => item.name).join(', ')]]; return <details className="profile-intelligence-details"><summary>Rozwiń szczegóły doświadczenia i kompetencji</summary><dl className="profile-review-list">{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || 'Brak danych'}</dd></div>)}</dl></details> }
