-- r7 makes the frozen offer snapshot and application-owned requirement manifest
-- part of the replay identity. Existing analysis history remains untouched and
-- becomes stale through the new prompt/algorithm versions.

alter table public.offer_versions
  add column if not exists analysis_contract_hash text;

alter table public.offer_versions
  add constraint offer_versions_analysis_contract_hash_format_check
  check (analysis_contract_hash is null or analysis_contract_hash ~ '^[0-9a-f]{64}$') not valid;

create index if not exists offer_versions_analysis_contract_hash_idx
  on public.offer_versions (user_id, analysis_contract_hash)
  where analysis_contract_hash is not null;
...
begin
  ...
  v_analysis_identity := public.workspace_analysis_identity(actor_id, p_offer_id, current_offer_version_id, current_profile_version_id, 'jobmatch-job-match-v3', 'gpt-5.4-mini', 'jobmatch-deterministic-r7:' || coalesce(v_contract_hash, 'pending'));
  ...
  if not p_force_reanalysis and v_contract_hash is not null then
    ...
    where v.user_id = actor_id and v.job_offer_id = p_offer_id and v.offer_version_id = current_offer_version_id
      ...
      and v.prompt_version = 'jobmatch-job-match-v3'
      and v.model_version = 'gpt-5.4-mini'
      and v.algorithm_version = 'jobmatch-deterministic-r7'
