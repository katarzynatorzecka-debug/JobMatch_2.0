-- R1.2/R1.3 correction: idempotent concurrent imports and parity for import state transitions.

create or replace function public.workspace_import_result(p_session_id uuid, p_idempotent boolean)
returns jsonb
language sql
security invoker
...
as $$
  select jsonb_build_object(
    ...
    'foundCount', s.found_count,
    'newCount', s.new_count,
    'duplicateCount', s.duplicate_count,
    'invalidCount', s.invalid_count,
    'needsReviewCount', s.needs_review_count,
    'status', s.status,
    'createdOfferIds', coalesce(s.operation_metadata -> 'createdOfferIds', '[]'::jsonb),
    'reusedOfferIds', coalesce(s.operation_metadata -> 'reusedOfferIds', '[]'::jsonb),
    'possibleDuplicateOfferIds', coalesce(s.operation_metadata -> 'possibleDuplicateOfferIds', '[]'::jsonb),
    'invalidItems', coalesce(s.operation_metadata -> 'invalidItems', '[]'::jsonb),
...
begin
  ...
  select s.id, s.status into session_id, existing_status
  from public.import_sessions s
  where s.user_id = actor_id and s.import_idempotency_key = import_key;

  if session_id is not null then
    if existing_status = 'reverted' then
      update public.import_sessions s
      set status = case when s.invalid_count > 0 then 'partial' else 'active' end,
          reactivated_at = now(),
          operation_metadata = s.operation_metadata || jsonb_build_object('reactivatedAt', now(), 'reactivatedByRetry', true)
      where s.id = session_id and s.user_id = actor_id;
    end if;
    select public.workspace_import_result(session_id, true) into result;
    return result;
  end if;
  ...
  on conflict (user_id, import_idempotency_key) where import_idempotency_key is not null do nothing
  returning id into session_id;

  if session_id is null then
    select s.id, s.status into strict session_id, existing_status
    from public.import_sessions s
    where s.user_id = actor_id and s.import_idempotency_key = import_key;
    if existing_status = 'reverted' then
      update public.import_sessions s
  ...
  update public.import_sessions s
  set status = case when v_invalid_count > 0 then 'partial' else 'active' end,
      new_count = v_new_count,
      duplicate_count = v_duplicate_count,
      invalid_count = v_invalid_count,
      needs_review_count = v_review_count,
      operation_metadata = jsonb_build_object('idempotencyKey', import_key, 'parserVersion', payload ->> 'parserVersion', 'invalidItems', coalesce(payload -> 'invalidItems', '[]'::jsonb), 'createdOfferIds', created_ids, 'reusedOfferIds', reused_ids, 'possibleDuplicateOfferIds', possible_ids)
  where s.id = session_id;
  select public.workspace_import_result(session_id, false) into result;
  return result;
end;
$$;

create or replace function public.workspace_revert_import(import_session_id uuid)
returns void language plpgsql security invoker set search_path = public as $$
begin
  update public.import_sessions s
  set status = 'reverted', reverted_at = now(), operation_metadata = s.operation_metadata || jsonb_build_object('revertedAt', now())
  where s.id = import_session_id and s.user_id = auth.uid() and s.status <> 'reverted';
  if found then return; end if;
  if exists (select 1 from public.import_sessions s where s.id = import_session_id and s.user_id = auth.uid() and s.status = 'reverted') then return; end if;
  raise exception 'WORKSPACE_IMPORT_NOT_FOUND';
end;
$$;

create or replace function public.workspace_reactivate_import(import_session_id uuid)
returns void language plpgsql security invoker set search_path = public as $$
begin
  update public.import_sessions s
  set status = case when s.invalid_count > 0 then 'partial' else 'active' end,
      reactivated_at = now(),
      operation_metadata = s.operation_metadata || jsonb_build_object('reactivatedAt', now())
  where s.id = import_session_id and s.user_id = auth.uid() and s.status = 'reverted';
  if found then return; end if;
  if exists (select 1 from public.import_sessions s where s.id = import_session_id and s.user_id = auth.uid() and s.status in ('active', 'partial')) then return; end if;
  raise exception 'WORKSPACE_IMPORT_NOT_FOUND';
end;
$$;
