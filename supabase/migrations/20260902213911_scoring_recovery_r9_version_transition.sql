-- The scoring recovery transition binds the corrected rubric and evidence semantics to one persisted
-- analysis identity. Existing versions remain untouched and become stale via
-- the frontend version comparison; ordinary replay can reuse only exact r9.
