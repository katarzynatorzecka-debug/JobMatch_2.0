-- r8 changes scoring semantics only: UNKNOWN remains outside score while
-- coverage/reliability continue to communicate incomplete evidence. Existing
-- history is preserved and becomes stale through the new versions.
...
begin
  ...
  v_analysis_identity := public.workspace_analysis_identity(actor_id, p_offer_id, current_offer_version_id, current_profile_version_id, 'jobmatch-job-match-v4', 'gpt-5.4-mini', 'jobmatch-deterministic-r8:' || coalesce(v_contract_hash, 'pending'));
  ...
  if not p_force_reanalysis and v_contract_hash is not null then
    ...
    where v.user_id = actor_id and v.job_offer_id = p_offer_id and v.offer_version_id = current_offer_version_id
      ...
      and v.prompt_version = 'jobmatch-job-match-v4'
      and v.model_version = 'gpt-5.4-mini'
      and v.algorithm_version = 'jobmatch-deterministic-r8'
