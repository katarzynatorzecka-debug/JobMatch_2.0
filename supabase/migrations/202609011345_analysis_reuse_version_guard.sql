-- A completed response may only be reused when its persisted contract matches
-- the identity contract currently requested by the queue.
-- This replaces the internal RPC body only; it does not alter historical rows.
...
begin
  ...
  if not p_force_reanalysis then
    ...
    where v.user_id = actor_id and v.job_offer_id = p_offer_id and v.offer_version_id = current_offer_version_id
      ...
      and v.prompt_version = 'jobmatch-job-match-v2'
      and v.model_version = 'gpt-5.4-mini'
      and v.algorithm_version = 'jobmatch-deterministic-r6'
