-- Failed queued attempts are terminal for the user's next explicit enqueue.
-- Keep their history, release the active-request slot, and prevent the worker
-- from racing a fresh request for the same offer/version/profile tuple.

drop index if exists public.analysis_queue_one_active_request_unique;

create unique index analysis_queue_one_active_request_unique
  on public.analysis_queue (user_id, job_offer_id, profile_version_id, offer_version_id, request_type)
  where status = 'processing' or (status = 'queued' and last_error is null);
...
begin
  ...
  v_analysis_identity := public.workspace_analysis_identity(actor_id, p_offer_id, current_offer_version_id, current_profile_version_id, 'jobmatch-job-match-v7-bilingual', 'gpt-5.4-mini', 'jobmatch-deterministic-r10-critical-priority:' || coalesce(v_contract_hash, 'pending'));
  ...
  perform 1 from public.analysis_queue q
  where q.user_id = actor_id and q.job_offer_id = p_offer_id
    and q.offer_version_id = current_offer_version_id and q.profile_version_id = current_profile_version_id
  for update;
  ...
  where user_id = actor_id and job_offer_id = p_offer_id
    and offer_version_id = current_offer_version_id and profile_version_id = current_profile_version_id
    and status = 'queued' and last_error is not null;
  if not p_force_reanalysis and v_contract_hash is not null then
    ...
    where v.user_id = actor_id and v.job_offer_id = p_offer_id and v.offer_version_id = current_offer_version_id
      ...
      and v.prompt_version = 'jobmatch-job-match-v7-bilingual'
      ...
      and jsonb_typeof(v.analysis_data -> 'localizedContent' -> 'pl') = 'object'
      and jsonb_typeof(v.analysis_data -> 'localizedContent' -> 'en') = 'object'
