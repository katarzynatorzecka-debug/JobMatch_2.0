...
const experienceEntry = fact({ role: { type: 'string', minLength: 1, maxLength: 180 }, company: { type: ['string', 'null'], maxLength: 160 }, startDate: { type: ['string', 'null'], maxLength: 40 }, endDate: { type: ['string', 'null'], maxLength: 40 }, duration: { type: ['string', 'null'], maxLength: 80 }, responsibilities: { type: 'array', maxItems: 30, items: fact({ capability: { type: 'string', minLength: 1, maxLength: 180 } }) }, achievements: { type: 'array', maxItems: 20, items: fact({ capability: { type: 'string', minLength: 1, maxLength: 300 } }) }, domains: { type: 'array', maxItems: 20, items: fact({ name: { type: 'string', minLength: 1, maxLength: 180 }, yearsApprox: { type: ['number', 'null'], minimum: 0, maximum: 60 } }) } })
const project = fact({ name: { type: 'string', minLength: 1, maxLength: 160 }, scope: { type: 'string', maxLength: 500 }, role: { type: 'string', minLength: 1, maxLength: 160 }, stack: { type: 'array', maxItems: 20, items: { type: 'string', minLength: 1, maxLength: 80 } }, result: { type: 'string', maxLength: 500 }, link: { type: ['string', 'null'], maxLength: 240 }, deployed: { type: ['boolean', 'null'] }, uxEvidence: { type: 'array', maxItems: 8, items: { type: 'string', minLength: 1, maxLength: 300 } }, prototypingEvidence: { type: 'array', maxItems: 8, items: { type: 'string', minLength: 1, maxLength: 300 } } })
const candidateFacts = {
  type: 'object', additionalProperties: false,
  required: ['totalExperienceYears', 'experienceEntries', 'projects', 'experienceAreas', 'skills', 'responsibilities', 'domains', 'achievements', 'languages', 'education', 'certifications'],
  properties: {
    totalExperienceYears: { type: 'object', additionalProperties: false, required: ['value', 'evidence', 'confidence', 'status'], properties: { value: { type: ['number', 'null'], minimum: 0, maximum: 60 }, evidence, confidence, status } }, experienceEntries: { type: 'array', maxItems: 30, items: experienceEntry },
    projects: { type: 'array', maxItems: 20, items: project }, experienceAreas: { type: 'array', maxItems: 20, items: fact({ area: { type: 'string', minLength: 1, maxLength: 180 }, yearsApprox: { type: ['number', 'null'], minimum: 0, maximum: 60 }, recency }) },
...
export function isCvProfileOutput(value: unknown): boolean {
  ...
  const factArrays = ['experienceEntries', 'projects', 'experienceAreas', 'skills', 'responsibilities', 'domains', 'achievements', 'languages', 'education', 'certifications']
