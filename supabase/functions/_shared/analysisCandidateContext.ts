...
export function buildAnalysisCandidateContext(profile: Record<string, unknown>, offerText: string) {
  ...
  return {
    ...
    experience: pick('experienceAreas', 'area', 8), skills: pick('skills', 'name', 20), capabilities: pick('responsibilities', 'capability', 12), domains: pick('domains', 'name', 8), achievements: pick('achievements', 'capability', 6), languages: pick('languages', 'name', 8), education: pick('education', 'name', 6), certifications: pick('certifications', 'name', 8),
