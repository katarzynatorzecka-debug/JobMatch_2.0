...
describe('candidate assessment contract', () => {
  it('requires every rubric criterion exactly once and preserves immutable fields', () => {
    ...
    expect(analysis.criteria.skills[0]).toMatchObject({ type: 'language', importance: 'critical', requiredExplicitly: true, matchType: 'no_evidence', outcome: 'NO_MATCH' })
  ...
  it('uses the full rubric and bounded candidate context in the assessment prompt', () => {
    ...
    expect(prompt).toContain('professional working proficiency in English is direct evidence for a B2 requirement')
